/* ------------------------------------------------------------------
   HexForge Engine – klassisches Skript (keine `import`‑Anweisung)
   ------------------------------------------------------------------ */

/* ---------- Konstanten & Hilfsfunktionen ---------- */
const BOARD_R = 3;                     // radius‑3‑Board
const HS = 34;                         // Hex‑Größe (Pixel)
...
/* ---------- Bewegungs‑/Angriffs‑Logik ---------- */
function validMoves(S,uid){
  const u=S.units[uid];
  if(!u||u.moved||u.atked) return [];
  const vis=new Set([cK(u.q,u.r)]), fr=[[u.q,u.r,u.s,0]], res=[];
  while(fr.length){
    const [q,r,s,d]=fr.shift();
    if(d>=u.bew) continue;
    cNbr(q,r,s).forEach(([nq,nr,ns])=>{
      if(!inB(nq,nr,ns))return;
      const k=cK(nq,nr);
      ...
      if(!cell||cell.type==='EMPTY'||cell.type==='WELL'){
        ...
        res.push([nq,nr,ns]); fr.push([nq,nr,ns,d+1]);
      }
    });
  }
  return res;
...
function validAtks(S,uid){
  ...
  Object.values(S.units).forEach(t=>{
    ...
    if(cDist([u.q,u.r,u.s],[t.q,t.r,t.s])<=u.rei) out.push({type:'unit',id:t.id,q:t.q,r:t.r,s:t.s});
  });
  const [bq,br,bs]= u.own==='A'?BASE_B:BASE_A;
  if(cDist([u.q,u.r,u.s],[bq,br,bs])<=u.rei) out.push({type:'base',own:u.own==='A'?'B':'A',q:bq,r:br,s:bs});
  return out;
}
function doAtk(S,aid,tt,tid){
  ...
  if(tt==='unit'){
    const d=S.units[tid];
    if(!d) return;
    d.hp-=a.atk; lg(S,a.own,`→${DB[d.cid].name}:-${a.atk}HP`);
    if(a.rei===1){ a.hp-=d.atk; lg(S,d.own,`↩Gegenschlag-${d.atk}HP`); }
    if(d.hp<=0){ lg(S,'sys',`${DB[d.cid].name} stirbt`); S.players[d.own].grave.push(d.cid); delete S.units[tid]; }
    if(a.hp<=0){ lg(S,'sys',`${DB[a.cid].name} stirbt`); S.players[a.own].grave.push(a.cid); delete S.units[aid]; return; }
  }else{
...
class Engine{
  ...
  _runPhase(){
    const S=this.S; if(S.winner){this._win();return;}
    this.selCard=null; this.selUnit=null; this.pact=null; this.vt=[];
    switch(S.phase){
      case 'DRAW':
        deal(S,this.ap,1); lg(S,this.ap,'Karte gezogen'); this._auto(350); break;
      case 'MANA':
      ...
      case 'CLEANUP':
        ...
        S.players[this.ap].boostUsed=false; S.lobDone=false;
        lg(S,this.ap,'— Runde Ende —');
        if(this.ap==='B') S.turn++;
        S.ap=this.ap==='A'?'B':'A'; S.phase='DRAW'; S.phaseStep=0;
        this._render(); setTimeout(()=>this._showSwitch(S.ap),200); return;
      default: break;
    }
    this._render();
  }
  ...
  _adv(){ const i=PHASES.indexOf(this.S.phase); this.S.phase=PHASES[i+1]||'CLEANUP'; this.S.phaseStep=i+1; this._runPhase(); }
  nextPhase(){ this._adv(); }
  ...
  _buildSVGMenu(){
    ...
    ring.setAttribute('cx',cx); ring.setAttribute('cy',cy); ring.setAttribute('r',H/2-8);
    ring.setAttribute('fill','url(#ringGrad)'); ring.setAttribute('stroke','#aa2525'); ring.setAttribute('stroke-width','4');
    svg.appendChild(ring);
    const ring2=document.createElementNS('http://www.w3.org/2000/svg','circle');
    ring2.setAttribute('cx',cx); ring2.setAttribute('cy',cy); ring2.setAttribute('r',H/2-20);
    ring2.setAttribute('fill','none'); ring2.setAttribute('stroke','rgba(180,50,50,0.3)'); ring2.setAttribute('stroke-width','1.5');
    svg.appendChild(ring2);

    const positions=[
      [cx,cy],                                   // 0 = Mitte (2×Neutral)
      [cx,cy-RR],                                // 1 oben
      [cx+RR*Math.sin(Math.PI/3),cy-RR*Math.cos(Math.PI/3)], // 2 oben‑rechts
      [cx+RR*Math.sin(Math.PI/3),cy+RR*Math.cos(Math.PI/3)], // 3 unten‑rechts
      [cx,cy+RR],                                // 4 unten
      [cx-RR*Math.sin(Math.PI/3),cy+RR*Math.cos(Math.PI/3)], // 5 unten‑links
      [cx-RR*Math.sin(Math.PI/3),cy-RR*Math.cos(Math.PI/3)]  // 6 oben‑links
    ];
  ...
  _selectMenuOpt(id){
    ...
    if(id==='_draw'){
      ...
      this._render(); this._auto(300); return;
    }
    if(id==='_boost'){
    ...
    /* alles andere → Zielzelle auswählen */
    this.selCard=id;
    this.pact='land';
  ...
  clickCell(q,r,s){
    ...
    if(this.pact && isV){
      if(this.pact==='land'){
        ...
        // Land‑Karten kommen vom Radial‑Menu, nicht aus der Hand → kein Hand‑Entfernen nötig
      ...
      if(this.pact==='atk' && this.selUnit){
        ...
        if(targetUnit && targetUnit.own!==this.ap) doAtk(S,this.selUnit,'unit',targetUnit.id);
  ...
  /* --------------------------------------------------------------
     Rendering (5 Teil‑Routinen)
     -------------------------------------------------------------- */
  _render(){ this._rStats(); this._rHand(); this._rPhase(); this._rBtns(); this._rLog(); this._rBoard(); }
  ...
  _rStats(){
    ...
    pn.textContent=`SPIELER ${S.ap}`; pn.className=`pn ${S.ap}`;
  }
  ...
  _rHand(){
    ...
    S.players[this.ap].hand.forEach(id=>{
      ...
      let stats='';
      if(cd.type==='UNIT'){
        stats=`<div class="cs"><span class="cra">⚔${cd.atk}</span><span class="crh">♥${cd.hp}</span><span style="color:var(--dim)">➤${cd.bew}</span><span style="color:var(--W)">◎${cd.rei}</span></div>`;
      }
  ...
  _rLog(){
    document.getElementById('lp').innerHTML=
      this.S.log.slice(0,30).map(e=>`<div class="le"><span class="l${e.p==='sys'?'S':e.p}">[${e.p}]</span>${e.m}</div>`).join('');
  }
  ...
  _rBoard(){
    ...
    function corners(cx,cy){
      return Array.from({length:6},(_,i)=>{const a=Math.PI/180*(60*i);return [cx+HS*Math.cos(a),cy+HS*Math.sin(a)];});
    }
    function pts(cx,cy){
    ...
    const defs=document.createElementNS('http://www.w3.org/2000/svg','defs');
    defs.innerHTML=`<filter id="gw"><feGaussianBlur stdDeviation="2.5" result="b"/>
        <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
      <filter id="wg"><feGaussianBlur stdDeviation="5" result="b"/>
        <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>`;
    svg.appendChild(defs);

    const vtSet=new Set(this.vt.map(v=>cK(v[0],v[1])));   // Felder die gerade hervorgehoben werden
    ...
    cells.forEach(([q,r,s])=>{
      ...
      if(cell && cell.type==='WELL'){
        ...
        orb.setAttribute('pointer-events','none'); g.appendChild(orb);
        ...
        if(ctrl){
          ...
          lbl.setAttribute('pointer-events','none');
          lbl.textContent=ctrl; g.appendChild(lbl);
        }
      }
      ...
      if(cell && cell.type==='BASE'){
        ...
        txt.setAttribute('pointer-events','none');
        txt.textContent=cell.owner; g.appendChild(txt);
        ...
        hp.textContent=`${S.players[cell.owner].hp}HP`; g.appendChild(hp);
      }
      ...
      if(cell && cell.type==='LAND'){
        ...
        txt.setAttribute('pointer-events','none');
        txt.textContent=em; g.appendChild(txt);

        const dot=document.createElementNS('http://www.w3.org/2000/svg','circle');
        dot.setAttribute('cx',cx+HS*.56); dot.setAttribute('cy',cy-HS*.56);
        dot.setAttribute('r','4');
        dot.setAttribute('fill',cell.owner==='A'?'rgba(200,60,60,.65)':'rgba(60,100,200,.65)');
        dot.setAttribute('pointer-events','none'); g.appendChild(dot);
      }

      /* Highlight für mögliche Aktionen */
      if(vtSet.has(k)){
